...
ggplot(table_1_select_tidy_national, 
       aes(area = percentage, fill = acqui_mode, 
           label = paste(acqui_mode, " (", percentage,"%",")", sep =""))) +
           geom_treemap() +
  labs(fill = "Property\nacquisition\nmode") +
...
ggsave("images/acqui_national_construct/treemap_acqui_plot.png", width = 12, height = 8)
...
ggsave("images/acqui_national_construct/donut_acqui_plot_urb.png", width = 5, height = 5)

# Rural Property Acquisition Mode

ggplot(table_1_select_tidy_rur, aes(x = hsize, y = percentage, fill = acqui_mode)) +
  geom_col(color = "black") +
  geom_text_repel(aes(label = paste(percentage, "%" , sep ="")),
...
ggsave("images/acqui_national_construct/donut_acqui_plot_rur.png", width = 5, height = 5)
